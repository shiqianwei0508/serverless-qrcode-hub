...
- **校验逻辑**（任一失败 `throw new Error`）：
  ...
  2. `banPath.includes(path)` → `RESERVED_PATH`（"该短链名已被系统保留，请使用其他名称"）。
  3. `expiry` 存在且 `isNaN(Date.parse(expiry))` → `INVALID_EXPIRY`（"Invalid expiry date"，仅校验可解析，不校验是否过去）。
  4. `isWechat && !qrCodeData` → `WECHAT_REQUIRES_QR`（"微信二维码必须提供原始二维码数据"）。
...
3. `fetch('/api/mapping', POST, JSON.stringify({ name,path,target,expiry,enabled,isWechat, qrCodeData: isWechat ? qrCodeData : null }))`：
   ...
   - 成功 → `showAlert(I18N.t('app.addSuccess'), 'success')` + `location.reload()`（整页刷新重新加载列表）。
